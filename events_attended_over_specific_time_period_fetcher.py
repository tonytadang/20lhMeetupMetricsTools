print '#####################################################'
print '# EVENTS ATTENDED OVER SPECIFIC TIME PERIOD FETCHER #'
print '#####################################################'
print ''
...
######################################################
# PART3- Get list of attendees (via their member_id) #
######################################################

meetup_api_key = '4b5e1025523d10283f602d7056327875'

#creates list of attendees_ids for all meetups within epoch timestamps:
attendees_ids = []

#populates attendees_ids list:
for event_id in event_ids:
	url = 'https://api.meetup.com/20letshang/events/'+event_id+'/attendance?key='+meetup_api_key+'&sign=true&photo-host=public&page=200'
	data = json.load(urllib2.urlopen(url))
	for item in data:
		attendees_ids.append(item['member']['id'])

####################################################################
# PART3- Get list of all_members_names and all_members_ids in 20LH #
####################################################################
...
for member_id in all_members_ids:
	events_attended.append(attendees_ids.count(member_id))

#######################################################################################
# PART5- Print event_ids, all_member_names, all_member_ids, and events_attended lists #
#######################################################################################
...
print 'COUNT OF EVENTS ATTENDED FOR EACH MEMBER:'
for count in events_attended:
	print count
